...
def eval_model(u_emb, i_emb, Rtr, Rte, k):
    ...
    for ue_f, tr_f, te_f in zip(ue_splits, tr_splits, te_splits):
        ...
        src = torch.ones(test_indices.numel()).reshape(test_indices.shape)
        pred_items.scatter_(dim=1,index=test_indices,src=src.cuda())

        topk_preds = torch.zeros_like(scores).float()
        topk_preds.scatter_(dim=1,index=test_indices[:, :k],src=src.cuda())

    
        TP = (test_items * topk_preds).sum(1)
        rec = TP/test_items.sum(1)
        ndcg = compute_ndcg_k(pred_items, test_items, test_indices, k)
        # rmse = torch.sqrt(torch.mean((topk_preds - test_items)**2))
